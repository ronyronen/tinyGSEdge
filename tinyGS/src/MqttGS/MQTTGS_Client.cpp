...
void MQTTGS_Client::loop() {
    if (!connected()) {
        status.mqttgs_connected = false;
        ...
        connectionAtempts = 0;
        status.mqttgs_connected = true;
    }

    if (connectionAtempts > connectionTimeout) {
        Log::console(PSTR("Unable to connect to MQTTGS Server after many atempts. Restarting..."));
        ESP.restart();
    }
...
void MQTTGS_Client::reconnect() {
    ConfigManager &configManager = ConfigManager::getInstance();
    uint64_t chipId = ESP.getEfuseMac();
    char clientId[13];
    sprintf(clientId, "%04X%08X", (uint16_t) (chipId >> 32), (uint32_t) chipId);

    Log::console(PSTR("Attempting MQTTGS connection..."));
    Log::console(
            PSTR("If this is taking more than expected, connect to the config panel on the ip: %s to review the MQTTGS connection credentials."),
            WiFi.localIP().toString().c_str());
    if (connect(clientId, configManager.getMqttGSUser(), configManager.getMqttGSPass(),
                buildTopic(teleTopic, topicStatus).c_str(), 2, false, "0")) {
        yield();
        Log::console(PSTR("Connected to MQTTGS!"));
        status.mqttgs_connected = true;
        subscribeToAll();
        sendWelcome();
    } else {
        status.mqttgs_connected = false;
        Log::console(PSTR("Failed to Connect to MQTTGS!"));
...
String MQTTGS_Client::buildTopic(const char *baseTopic, const char *cmnd) {
    ConfigManager &configManager = ConfigManager::getInstance();
    String topic = baseTopic;
    topic.replace("%user%", configManager.getMqttGSUser());
    topic.replace("%station%", configManager.getThingName());
    topic.replace("%cmnd%", cmnd);

    Log::console(PSTR("buildTopic %s"), topic.c_str());

    return topic;
}

void MQTTGS_Client::subscribeToAll() {
    subscribe(buildTopic(globalTopic, "#").c_str());
    subscribe(buildTopic(cmndTopic, "#").c_str());
}

void MQTTGS_Client::sendWelcome() {
...
void MQTTGS_Client::sendRx(String packet, bool noisy) {
    ConfigManager &configManager = ConfigManager::getInstance();
    time_t now;
    time(&now);
    struct timeval tv;
...
void MQTTGS_Client::sendAdvParameters() {
    ConfigManager &configManager = ConfigManager::getInstance();
...
void MQTTGS_Client::manageMQTTGSData(char *topic, uint8_t *payload, unsigned int length) {
    Log::console(PSTR("manageMQTTGSData"));
    ...
    if (!strcmp(command, commandBegine)) {
        size_t size = JSON_ARRAY_SIZE(10) + 10 * JSON_OBJECT_SIZE(2) + JSON_OBJECT_SIZE(16) + JSON_ARRAY_SIZE(8) +
                      JSON_ARRAY_SIZE(8) + 64;
...
void MQTTGS_Client::manageSatPosOled(char *payload, size_t payload_len) {
...
void MQTTGS_Client::remoteSatCmnd(char *payload, size_t payload_len) {
...
void MQTTGS_Client::remoteSatFilter(char *payload, size_t payload_len) {
    DynamicJsonDocument doc(256);
    deserializeJson(doc, payload, payload_len);
...
void MQTTGS_Client::remoteGoToSleep(char *payload, size_t payload_len) {
    DynamicJsonDocument doc(60);
    deserializeJson(doc, payload, payload_len);
...
void MQTTGS_Client::remoteSetFreqOffset(char *payload, size_t payload_len) {
    DynamicJsonDocument doc(60);
    deserializeJson(doc, payload, payload_len);
...
void manageMQTTGSDataCallback(char *topic, uint8_t *payload, unsigned int length) {
    Log::console(PSTR("Received MQTTGS message: %s : %.*s"), topic, length, payload);
    MQTTGS_Client::getInstance().manageMQTTGSData(topic, payload, length);
}

void MQTTGS_Client::begin() {
    ConfigManager &configManager = ConfigManager::getInstance();
    setServer(configManager.getMqttGSServer(), configManager.getMqttGSPort());
    setCallback(manageMQTTGSDataCallback);
}
